...
class MOGP(ExactGP, MultiTaskGPyTorchModel):
    ...
    def get_all_tasks(
        ...
        if train_X.ndim != 2:
            ...
            raise ValueError("Unsupported shape {} for train_X.".format(train_X.shape))
        d = train_X.shape[-1] - 1
        if not (-d <= task_feature <= d):
            raise ValueError("Must have that -{} <= task_feature <= {}".format(d, d))
    ...
    def construct_inputs(cls, training_data: TrainingData, **kwargs) -> Dict[str, Any]:
        ...
        if prior and prior_config:
            raise ValueError("Only one of `prior` and `prior_config` arguments expected.")
        ...
        if prior_config:
            ...
            if not isinstance(eta, float) and not isinstance(eta, int):
                raise ValueError("eta must be a real number, your eta was {}.".format(eta))
...
class RandomForestModel(RandomForestRegressor):
    ...
    def predict(self, X, return_std=False):
        if return_std:
            ...
            return torch.Tensor(ypred_mean), torch.Tensor(ypred_std)



        else:
            return torch.Tensor(super(RandomForestModel, self).predict(X))
