...
class DesignSpaceExplorationProblem(object):
    def __init__(self, path_dataset: str, num_objectives: int = 2):
        ...
        self._ref_points = torch.Tensor([0 for i in range(num_objectives)])
    ...
    def load_dataset(self,path_dataset: str, scale: bool = True):
        ...
        self.total_x = torch.Tensor(dataset[:, :-2])
        self.total_y = torch.Tensor(dataset[:, -2:])
    ...
    def evaluate(self, x: torch.Tensor):
        ...
        value = self.y[indices].to(torch.double).squeeze()
    ...
    def calc_adrs(self, y: torch.Tensor):
        ...
        for omega in reference:
            mini = float('inf')
            for gama in get_pareto_front(y):
    ...
    def calc_hv(self, y: torch.Tensor):
        return pg.core.hypervolume(-1.0*np.array(get_pareto_front(y))).compute(np.array(self._ref_points))
...
if __name__ == '__main__':
    problem = DesignSpaceExplorationProblem("./dataset/TRAM.csv")
